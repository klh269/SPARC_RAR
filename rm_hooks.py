...
from utils_analysis.MOND_IFs import n_family, delta_family, gamma_family, alpha_family
...
def g_obs_fit(table, i_table, data, bulged, family:str, pdisk:float=pdisk, pbul:float=pbul):
    ...
    param = sample("IF_param", dist.Uniform(0.0, 10.0))

    if family == "n": g_obs = deterministic("g_obs", n_family(param, g_bar))
    elif family == "delta": g_obs = deterministic("g_obs", delta_family(param, g_bar))
    elif family == "gamma": g_obs = deterministic("g_obs", gamma_family(param, g_bar))
    elif family == "alpha": g_obs = deterministic("g_obs", alpha_family(param, g_bar))
...
if __name__ == "__main__":
    SPARC_data, _, galaxy_count = get_SPARC_data()
    galaxies = [
    ...
    for gal in galaxies:
        ...
        mcmc.run(random.PRNGKey(0), table, i_table, SPARC_data[gal]["data"], SPARC_data[gal]["bulged"], "n")
        mcmc.print_summary()
        ...
        best_param = samples["IF_param"][max_ll_idx]
        results.append((gal, float(best_param)))

        # Plot RAR with best-fit parameter.
        plt.scatter(g_bar, g_obs, s=10, color='k', label='RAR with max log-likelihood')
        plt.xscale('log')
        plt.yscale('log')
        ...
        plt.legend()
        plt.savefig(f"/mnt/users/koe/SPARC_RAR/plots/rm_hooks/{gal}_n_family.png", dpi=300)

        jax.clear_caches()

    for gal, param in results:
        print(f"{gal}: best-fit n-family parameter = {param:.2f}")
